...
#if 0
void jsonInput(std::istream &input, std::vector <GroceryItem> &items) // when user enters 'json'
{
        GroceryItem groceryItem; // create a grocery item object
        do
        {
                std::cout << "Please enter the string in json format:\n" << std::endl;
                        input >> groceryItem; // loop through the vector and append values to different values
                if (input.rdstate() == std::ios_base::failbit) // check the current state of input stream
                {
        ...
        while (input.rdstate() == std::ios_base::failbit); // keep looping until the parsing succeeds
...
void jsonInput(std::istream &input, std::vector <GroceryItem> &items) // when user enters 'json'
{
        GroceryItem groceryItem; // create a grocery item object
//        std::cout << "Please enter the string in json format:\n" << std::endl;
        while (input >> groceryItem)
        {
                items.push_back(groceryItem); // add a populated grocery item object to the vector
        }
        if (!input.eof())
        {
                throw std::runtime_error("Error while reading JSON data");
        }
}
#endif
...
void GroceryItem::list(std::vector <GroceryItem> &items, std::string &user_input)
{
        auto discount = stoi(user_input);
        std::string json_str;
        for (auto &s : items)
        {
                if (s.item_discount>=discount)
                {
                        std::cout << s; //loop through the vector and use overloaded operator to print the item info
                }
        }
}